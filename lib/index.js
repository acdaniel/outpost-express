...
module.exports = function outpostExpress (services, formatters) {
  ...
  return [
    ...
    function renderEntity (req, res, next) {
      ...
      if (format) {
        formatters[format].formatEntity(res, options);
      } else {
        formatters['json'].formatEntity(res, options);
      }
    },
    function (err, req, res, next) {
      ...
      var format = req.accepts(_.keys(formatters));
      if (format) {
        formatters[format].formatError(err, res, options);
      } else {
        formatters['json'].formatError(err, res, options);
      }
    }
  ];
};
